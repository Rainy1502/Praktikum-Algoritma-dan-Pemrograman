    ...
    switch (pilihan) {
      ...
      case 2:
		...
		if (nominal > 0)
		{
			...
			printf("Nomor rekening : %s\n", no_rek);
            printf("Nama Akun : %s\n", nama_akun);
        	  printf("Setoran berhasil dilakukan. Saldo Anda sekarang: Rp. %.0f\n", saldo);
            getchar();
		} else {
      ...
      case 3:
		...
		if (nominal > 0) {
			if (saldo - nominal < 50000) {
				printf("Saldo Anda tidak mencukupi. Saldo minimal yang harus tersisa adalah Rp. 50.000\n");
			 } else {
				printf("Nomor rekening : %s\n", no_rek);
				printf("Nama Akun : %s\n", nama_akun);
				printf("Penarikan tunai berhasil dilakukan. Saldo Anda sekarang: Rp. %.0f\n", saldo);
        getchar();
			}  	
    }
        break;
